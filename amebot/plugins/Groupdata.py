...
from userbot.utils import  errors_handler, amebot_cmd

@borg.on(amebot_cmd(pattern="leave$"))
async def leave(e):
        await e.edit("`Me iz leaving this chat group.....!Goodbye aren't forever..` ")
        time.sleep(3)
        if '-' in str(e.chat_id):
...
@borg.on(amebot_cmd(pattern="chinfo(?: |$)(.*)", outgoing=True))
...
@borg.on(amebot_cmd(pattern="adminlist", outgoing=True))
@errors_handler
...
@borg.on(amebot_cmd(pattern=r"users ?(.*)", outgoing=True))
